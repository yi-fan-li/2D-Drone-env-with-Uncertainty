...
import numpy as np
import pygame
# referenced from https://github.com/marek-robak/Drone-2d-custom-gym-env-for-reinforcement-learning/blob/main/drone_2d_custom_gym_env_package/drone_2d_custom_gym_env/Drone.py
class Drone_physic():

    def __init__(self, x, y, angle, height, width, mass_f, mass_m, space):
        ...
        left_motor_moment_of_inertia = pymunk.moment_for_poly(mass_m, self.left_motor_shape.get_vertices())

        left_motor_body = pymunk.Body(mass_m, left_motor_moment_of_inertia, body_type=pymunk.Body.DYNAMIC)
        ...
        right_motor_moment_of_inertia = pymunk.moment_for_poly(mass_m, self.right_motor_shape.get_vertices())

        right_motor_body = pymunk.Body(mass_m, right_motor_moment_of_inertia, body_type=pymunk.Body.DYNAMIC)
